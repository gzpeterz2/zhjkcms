...
<script type="text/javascript">
	...
	//打开添加企业信息
	function openMastersAddDialog() {
		$("#dlg").dialog("open").dialog("setTitle", "添加企业简介");
		url = "master/save.action";
	}
	//打开修改企业信息框
	function openMastersModifyDialog() {
		...
		$("#m_id2").val(row.m_id);
		$("#m_name2").val(row.m_name);
		if(row.status==1){
			$("#status1").attr("checked","checked");
		}else{
			$("#status0").attr("checked","checked");
		}
		$("#m_photos2").val(row.m_photos);
		$("#photos2").val("");
		if(row.m_intro==null){
			editor.txt.html("");
		}
		editor.txt.html(row.m_intro);
	...
	function deleteMaster() {
		...
		for (var i = 0; i < selectedRows.length; i++) {
			strIds.push(selectedRows[i].m_id);
		}
		var ids = strIds.join(",");
		$.messager.confirm("系统提示", "您确认要删掉这<font color=red>"
				+ selectedRows.length + "</font>条数据吗？", function(r) {
			if (r) {
				$.post("${pageContext.request.contextPath}/master/delete.action", {
					delIds : ids
				}, function(result) {
					if (result.success) {
	...
	function showimages() {
		...
		document.getElementById('imgInit').src = row.m_photos;
	}
	...
	function closeMastersDialog() {
		$("#dlg").dialog("close");
		resetValue();
	}

	function closeMastersDialog1() {
		$("#dlg1").dialog("close");
		resetValue();
	}
...
<body>
	...
	<div id="tb">
		<div>
			<a href="javascript:openMastersAddDialog()" class="easyui-linkbutton"
				iconCls="icon-add" plain="true">添加
			</a>
			<a id="toUpdate" href="javascript:openMastersModifyDialog()" class="easyui-linkbutton"
				iconCls="icon-edit" plain="true">修改
			</a>
			<a href="javascript:deleteMaster()" class="easyui-linkbutton"
	...
	<table id="dg" title="公司简介" class="easyui-datagrid" fitColumns="true"
		...
		url="master/selectByPage.action" toolbar="#tb"  striped=true autoRowHeight=false>
		<!--  fitColumns="true" th自适应宽度； pagination：翻页；rownumbers：添加行号；url：必须返回json形式 -->
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="m_id" width="10" hidden=true>m_id</th>
				<th field="m_name" width="15">大咖姓名</th>
				<th field="m_photos" width="15">大咖照片</th>
				<th field="m_intro" width="15">大咖简介</th>
				<th field="status" width="15">是否显示</th>
			</tr>
		</thead>
	</table>
			...
			<table cellspacing="5px;">
				...
				<tr>
					...
					<td><input type="file" name="photos" id="photos"
						class="easyui-validatebox" required="true" />
					<input type="hidden" name="m_photos" id="m_photos"/>
					</td>
				</tr>
				<tr>
					<td>是否展示：</td>
				...
				<tr>
					<td colspan="4">
						<textarea id="text2" name="m_intro" style="width:100%; height:200px;display: none;"></textarea><!-- display: none; -->
					</td>
				</tr>
			</table>
		</form>
		...
		<form id="fm1" method="post" enctype="multipart/form-data">
			<table cellspacing="5px;">
				<tr>
					<td height="15px"></td>
					<!-- id -->
					<td><input type="hidden" name="m_id" id="m_id2"/></td>
				</tr>
				<tr>
					<td>大咖姓名：</td>
					<td><input type="text" name="m_name" id="m_name2"
						class="easyui-validatebox" required="true" />
					</td>
				</tr>
				<tr>
					<td>大咖照片：</td>
					<td><input type="file" name="photos" id="photos2"/></td>
					<td><input type="hidden" name="m_photos" id="m_photos2"/></td>
				</tr>
				<tr>
					<td>是否展示：</td>
					<td>
				...
				<tr>
					<td colspan="4">
						<textarea id="text1" name="m_intro" style="width:100%; height:200px;display: none;"></textarea><!-- display: none; -->
					</td>
				</tr>
			</table>
		</form>
	...
	<div id="dlg-buttons">
		<a href="javascript:saveMaster()" class="easyui-linkbutton"
			iconCls="icon-ok">保存</a> <a href="javascript:closeMastersDialog()"
			class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>

	<div id="dlg-buttons1">
		<a href="javascript:saveMaster1()" class="easyui-linkbutton"
			iconCls="icon-ok">保存</a> <a href="javascript:closeMastersDialog1()"
			class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>
...
<!-- 		<form id="uploadImg" method="post" action="Masters!uploadPhoto" -->
...
<!-- 		</form> -->
	...
	<script type="text/javascript">
		...
		document.getElementById('toUpdate').addEventListener('click', openMasterModifyDialog(editor), false)
	</script>
